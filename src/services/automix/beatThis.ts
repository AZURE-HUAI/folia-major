...
//
// The network itself is not here. It runs in the Electron main process (electron/analysis/), so
// this file stays pure, testable without an audio device, and free of any model dependency - the
// same rule the rest of the evidence layer follows.
//
// Every constant below is a CONTRACT, not a tuning knob. They are the values the checkpoint was
// trained with, read off the reference C++ port (mosynthkey/beat_this_cpp, MIT), and changing any
// of them does not make the answer slightly different - it makes the model confidently wrong,
// which is the failure mode this codebase has been bitten by three times already. If a beat grid
// ever looks subtly off, verify against `beats.json` before touching a number here.
...
/**
 * Longest this may hold the main thread between yields. The same budget `analyseTrack` uses, and
 * for the same reason - half a frame of animation.
 */
 ...
 * window at each end so frame `i` is centred on sample `i * HOP` rather than starting there. That
 * is load-bearing rather than cosmetic - without it every beat this returns is half a window late,
 * which is 23ms, which is most of the tolerance the whole grid is judged on.
 *
 * Async for the same reason `analyseTrack` is, and it is not optional: measured in the renderer,
 * this is 682ms in ONE block on a four-minute track and 1017ms on a six-minute one. That is not a
 * slow function, it is a frozen window - the lyrics stop, the visualiser stops - once per analysed
 * track. `analyseTrack` had already been through exactly this and carries the fix; this arrived
 * later and did not, which is how a stutter that had been diagnosed and closed came back.
 */
export const melSpectrogram = async (mono: Float32Array): Promise<MelSpectrogram> => {
    ...
    for (let frame = 0; frame < frames; frame += 1) {
        ...
        // amount of work the last time this bug was fixed - see YIELD_BUDGET_MS in trackProfile.
 ...
 * speaks.
 *
 * Some accuracy is given up here and it is worth naming: the model returns where each beat ACTUALLY
 * is, and a track that drifts has no single period. But `barGrid`, `snapToGrid` and `alignEntry`
 * all take an offset and a period, and the measured bar jitter on a real library is under one
 * percent of a bar - so a uniform fit costs a couple of milliseconds while a representation change
 * would touch every consumer. Revisit only if something downstream starts caring about drift.
 *
 * The MEDIAN interval, not the mean: a missed beat produces one interval of twice the period, and a
 * mean would spread that across the whole track. The median does not notice it.
 *
 * Anchored on the LAST beat for the same reason the estimator it replaces is - the outgoing track
 * is the one a handover is scheduled against, and that is its end. `headDownbeatOffset` is the same
 * bar line measured at the other end, for the incoming side.
 */
...
/**
 * Once the model has declined once it is not asked again for the rest of the session.
 *
 * The three ways it declines - not Electron, no weights file, runtime failed to start - are all
 * permanent for the life of the process, and retrying would put an IPC round trip and a failed
 * native load in front of every track the prefetcher touches.
 */
let modelDeclined = false;

/** For tests and for a settings page that wants to say whether the model is doing anything. */
export const beatThisDeclined = () => modelDeclined;

/**
 * Whether this build has the model at all.
 *
 * The desktop/browser split in one property lookup: the bridge only exists in Electron, so a web
 * build is permanently on the built-in estimator and should say so rather than appearing broken.
 */
export const canRunBeatThis = (): boolean =>
    !modelDeclined && typeof window !== 'undefined' && typeof window.electron?.runBeatThis === 'function';

/**
 * The whole Beat This! pass: samples in, beat grid out.
 *
 * Null whenever the model is not there, which is not an error - it is the browser build, and it is
 * any desktop install whose weights are missing. Every caller must treat null as "use the estimator
 * you already had", because that estimator is still the only answer in those environments.
 *
 ...
 */
export const analyseBeatGrid = async (mono: Float32Array): Promise<BeatGrid | null> => {
    const run = typeof window !== 'undefined' ? window.electron?.runBeatThis : undefined;
    if (modelDeclined || typeof run !== 'function') return null;

    const mel = await melSpectrogram(mono);
